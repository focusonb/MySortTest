...
void quicksort1(T *array, int left, int right, bigger bigger_int = bigger(), less less_int = less())
{
    if (left >= right)
        return;
    ...
    while (1)
    {
        ...
        {
            leftCursor++;
        } while (less_int(array[leftCursor], pivot));
        do
        {
            rightCursor--;
        } while (bigger_int(array[rightCursor], pivot));
    ...
    quicksort1<T, Bigger_int, Less_int>(array, left, rightCursor - 1);
    quicksort1<T, Bigger_int, Less_int>(array, rightCursor + 1, right);
};
